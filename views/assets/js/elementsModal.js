...
var HOST_URL = "https://www.awsom.me";
        ...
        <div class="ElementsModal--forms">
          <div class="ElementsModal--form">
            <label for="ElementsModal--card-element">
              ...
              <div style="width: 20em" class="StripeElement" id="card-element">
                <!-- A Stripe Element will be inserted here. -->
              </div>
            </label>
            <!-- Used to display form errors. -->
            <div id="card-error" class="ElementsModal--error-message" role="alert"></div>
          </div>
          <div class="ElementsModal--form">
            <label class="select">
          ...
          <div class="ElementsModal--form">
            ...
            <button id="button-text" class="ElementsModal--pay-button">Pay ${amount}</button>
          </div>
          <!-- Edit your terms and conditions here   -->
          <div class="footer ElementsModal--footer-text">
            By making this purchase, you agree to Awsom's Terms and Conditions<br>
            This Payment Gateway is Powered by Stripe
          </div>
        </div>
      </div>
    </form>
    ...
    stripe
      ...
      .then(function(result) {
        if (result.error) {
          ...
          orderComplete(result.paymentIntent.id, content.gigid, content.customerEmail, content.customerName, content.customerPhone, content.specialRequest, content.gigprice);
        }
      });
  });
}
...
var orderComplete = function(paymentIntentId, gigid, customerEmail, customerName, customerPhone, specialrequest, price) {
  loading(false);
  // check if direct mongoDB call can be made from here
      fetch(HOST_URL + "/placeorder/" + gigid, {
        method: "POST",
        headers: {
          "Content-Type": "application/json"
        },
        body: JSON.stringify({
          ...
          price: price,
          paymentCompleteId: paymentIntentId
        })
      }).then(function(response) {
